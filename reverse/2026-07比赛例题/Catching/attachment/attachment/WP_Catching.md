...
```
[*] 密文 (hex): 73796e7439097670722317796e742376662375726572231c6572236c6268177661712a2a2a36
[*] 明文 (hex): 666c61677b4e6963655f466c61675f69735f686572655f4172655f796f7546696e642121217d
[+] Flag: flag{Nice_Flag_is_here_Are_youFind!!!}
[+] 正向验证通过: 明文加密后与原始密文一致
```
...
1. **字符串提示** — `"welcome to rot reverse world!"` 中 **"rot"** 直接指明加密方式为 ROT 轮转
2. **提取密文** — `strings` 或 IDA 中定位 38 字节硬编码比较数据 (`0x404224`)
3. **分析变换函数** — 反编译验证函数发现逐字符处理逻辑：
   - `islower()` → `addb $0x9F` → ROT13（小写字母标准轮转）
   - `isupper()` → `addb $0xBF` → 大写专用映射，输出控制字符
   - `{_}!` → `9#*`（flag 格式符替换）
   - 其余字符原样通过
4. **逐字节逆向** — 利用映射关系（ROT13 自逆 + 大写查表 + 特殊符号互换）对密文逐字节还原
5. **验证** — 加密-解密一致性验证确保正确

### 关键经验

- **区分大小写两套逻辑**：小写使用标准 ROT13，大写使用另一套映射产生非字母输出。不能简单认为所有字母都是 ROT13
- **自逆性质利用**：小写 ROT13 和特殊符号映射都是自逆的（`f↔s`, `{↔9` 等），同一函数即可正向加密也可逆向解密
- **汇编细节**：`addb $0x9F` 和 `addb $0xBF` 分别对应 `-0x61` 和 `-0x41`（求字母表索引），之后各自走不同的处理分支
