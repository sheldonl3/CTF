...
题目给出一个 ELF **core dump（进程内存快照）**。程序把 flag 经「篡改版 RC4 + 位置相关自定义 base64」加密后与常量 `TGT` 比对，而 `KEY`/`TAB`/`TGT` 三个缓冲区在内存中都被 XOR 混淆。需要从 core 里提取并解码这三个缓冲，逆向出被改过的加密算法，再反推回 flag。

## Solution

### Step 1: 解析 core，定位加密流程并提取三个被混淆的缓冲区

`dc.core` 是内存快照，代码在 `PT_LOAD` 段里带虚拟地址。用 capstone 反汇编 `main`（基址 `0x55c62414e000`，函数 `main@0x186c`）可确认流程：

```
读入 flag -> RC4(flag, KEY) -> base64(cipher) -> strcmp(结果, TGT)
```

即 **`TGT = b64(RC4(flag))`**，所以 **`flag = RC4(b64_decode(TGT))`**。
三个缓冲在内存里是 XOR 混淆（anti-forensics）：`KEY` 异或 `0x56`、`TAB` 异或 `0x42`、`TGT` 异或 `0x24`，必须先解码。
反汇编 `RC4`（KSA@0x1370、PRGA@0x145c）和 `base64`@0x16d5，逐条核对出篡改点：

- KSA：`j = (j + S[i] + key[i%len] + i) & 0xff` —— 比标准 RC4 **多了 +i**
- PRGA 密钥流：`ks = k ^ ((S[i] + S[j]) & 0xff)` —— 把**输出字节位置 k** 异或进密钥流，且用 **S[i]+S[j] 之和**（非标准的 `S[(S[i]+S[j])]`）
- base64：`out[pos] = TAB[(idx + (pos%7) + 3) % 64]`，`pos` 为**累计输出位置**

下面是从 core 文件直接跑到 flag 的完整脚本（含正反验证）：
...
运行输出（验证 `b64(RC4(flag)) == TGT` 为 `True`）：

```
KEY   : b'C0nFu5i0n_K3y!'
TGT   : b'W;1C_a@mM^DWqDOYcE%,LtCl9:BOO$S,MO,%;}$5NZ5^;G%I%H>H'
cipher: 6d194d02da60b3c2575063b6a5e7c0d793fd7fd488386482a49fe34481342aee3d42b7cc75fe5d (39 bytes)
FLAG  : flag{Tw34k3d_RC4_n_B64_m4k3_1t_h4rd!!!}
